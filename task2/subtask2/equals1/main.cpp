...
main (int argc, char * argv [])
{
  ...
  if (argc != 7)
    {
      fprintf (stderr, "Use: %s task_No Mu C M tau eps\n", argv[0]);
      return -1;
    }
  ...
  for (int m = 0; m <= M; m++)
    {
      double x = m * params.h;
      V_curr[m] = (task == 1) ? u_1 (x) : u_2 (x);
      G_curr[m] = (task == 1) ? g_1 (x) : g_2 (x);
    }
...
rho_1 (double x)
{
  return (4.5 <= x && x <= 5.5) ? 2. : 1.;
}

double
g_1 (double x)
{
  return (4.5 <= x && x <= 5.5) ? M_LN2 : 0.;
}
...
{
//  double C = data.C;
  return data.C/*(C < 0) ? 1.4 * exp (0.4 * g) : C*/;
}
...
scheme (const data_t & data, const params_t & params, arr_t & V_curr, arr_t & G_curr, arr_t & V_next, arr_t & G_next)
{
  ...
  for (int m = 0; m <= M; m++)
    init_mass += exp (G_curr[m]);
  init_mass *= data.X / M;

  for (int n = 0; ; n++)//change there
    {
      double t = n * tau;
...
//      double res_V = residual (V_curr, V_next, M);
//      double res_G = residual (G_curr, G_next, M);
//      double res = std::max (res_V, res_G);
      ...
      if (n % 1000 == 0)
        {
            printf ("%e ", (mass - init_mass) / init_mass);
        }
