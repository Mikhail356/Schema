        ...
        for mu in mu_set:
            ...
            for t_it in [1, 2]:
                for m_it in [1, 2]:
                    ...
                    Y = [to_num (t) for t in os.popen (s).read ().split ()][:-1]
                    X = [(tau / t_it) * n for n in range (len (Y))]

                    #n = int (len (Y) / 500)
                    #if n == 0:
                    #    n = 1
                    #x = [1000 * t for t in X[:-100:n]] + [1000 * t for t in X[-100::]]
                    #y = Y[:-100:n] + Y[-100::]
            ...
            plt.legend ()
            plt.savefig (f'plot_1_{No}_{k}.pdf')
            mu_pow = int (math.log10 (mu))
            if C > 0:
                C_pow = int (math.log10 (C))
            else:
                c_pow = 1.4
            print ('\\inserpicture{res/' + f'plot_1_{No}_{k}.pdf' + '}{Графики функций $R$ для $\mu = 10^{' + str (mu_pow) + '}$ и $C = 10^{' + str (C_pow) + '}$}')
            plt.cla ()
